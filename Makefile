...
# Load .env (if present) so conn/psql/info can read the values.
ifneq (,$(wildcard .env))
include .env
endif

.PHONY: help init up down reset conn env example psql info status logs test version garage-init tunnel tunnel-login
